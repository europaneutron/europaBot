-- Sustituir no puede barrer lo que el compilador no sabe reponer.
--
-- La version anterior desactivaba todas las intenciones del subarbol, y el
-- subarbol incluye el alcance de la corrida, que en el caso normal es la raiz.
-- Se llevaba por delante `saludo`, `despedida` y `cita`, que el material nunca
-- describe como preguntas y que ningun preset produce: tras publicar, el bot
-- dejaba de saludar y no podia agendar una visita.
--
-- La regla pasa a ser: se retira el contenido de las preguntas que esta
-- publicacion cubre. Una pregunta de la que el material no habla se queda como
-- estaba. No hace falta una lista de intenciones protegidas.
--
-- Y un alcance que la propia corrida creo se conserva aunque no le haya tocado
-- ninguna propuesta. Antes nacia y moria en la misma publicacion: un modelo
-- cuyos hechos no cubren ninguna pregunta desaparecia del arbol que el material
-- describe.
