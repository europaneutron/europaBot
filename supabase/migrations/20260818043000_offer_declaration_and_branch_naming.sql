-- Dos reglas nuevas de publicacion, del mismo tipo que la de vocabulario que
-- no alcanza su pregunta: una respuesta de si/no sin oferta declarada, y una
-- respuesta que cruza ramas sin nombrarlas. Las dos se bloquean con motivo
-- visible; el resto de la corrida se publica igual, que es como ya funciona
-- el bloqueo por vocabulario.
--
-- La oferta declarada viaja de la propuesta a la respuesta publicada, para
-- que el runtime sepa que ofrece un afirmativo cuando llegue.

ALTER TABLE public.compiler_proposals
  ADD COLUMN offers_intent_name VARCHAR(50);

COMMENT ON COLUMN public.compiler_proposals.offers_intent_name IS
  'Cuando la respuesta termina en pregunta de si/no, la intencion que un afirmativo del lead debe disparar.';
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.compiler_proposals (
      ...
      message_text, matcher_patterns, review_signals, is_publishable,
      review_details, offers_intent_name
    ) VALUES (
      run_uuid,
      ...
      NULLIF(item->>'offers_intent_name', '')
    )
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.bot_responses (
      ...
      edited_by_human, offers_intent_name
    ) VALUES (
      proposal.intent_id, intent.intent_name, proposal.response_key,
      proposal.message_text, 'fragmented', '{}'::jsonb, true, 1,
      'compiler', proposal.id, proposal.edited_by_human, proposal.offers_intent_name
    )
