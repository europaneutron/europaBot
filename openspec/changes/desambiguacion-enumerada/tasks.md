...
- [x] 10.5 Corrida real del compilador ejecutada el 18 de agosto de 2026 (`scripts/walkthrough-fymsa.ts`, llamada en vivo al modelo). Sobre el contenido compilado pasan: saludo con el negocio nombrado, "cuanto cuesta" preguntando con opciones, elegir un desarrollo, nombrar un modelo ("Solara" -> "Desde $2,980,000 MXN"), amenidades por herencia y agendar. La corrida destapó que repetir la última pregunta incluía `saludo`, corregido con prueba. Queda una sola frase sin cubrir, "donde estan ubicados": `ubicacion` está publicada en los dos desarrollos y el matcher sí reconoce "donde se ubica", así que es cobertura de vocabulario y pertenece a `vocabulario-del-matcher`, no a la desambiguación. Por eso 8.3-8.4 de `vocabulario-del-matcher` siguen abiertas; 9.3-9.4 de `material-sustituye` quedan cerradas.
